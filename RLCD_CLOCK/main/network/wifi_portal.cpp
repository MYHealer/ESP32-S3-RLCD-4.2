...
{
    // MiPlay/airkan 常驻期间 modem sleep 必须保持关闭：AP 会缓存组播帧，
    // 手机 mDNS 查询在 DTIM 间隙整条丢失，设备就无法被发现。此处只拦截
    // "想进 MAX_MODEM"的路径； portal（PS_NONE）本来就不省电，不拦。
    if (!enable_setup_portal && wifi_always_on_required()) {
        ESP_LOGD(TAG,
                 "wifi power save request deferred: discovery services active");
        return;
    }
                               ...
                               void *event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START &&
        ...
        // 拿到 IP 后才能启动 MiPlay 发现层：设备身份里的 A 记录、DebugInfo
        // 都依赖 STA 地址，早于此时启动会报告 127.0.0.1。
        // 该函数内部幂等，Wi-Fi 重连重复触发不会重复建任务。
        // MiPlay 与 airkan 都是常驻可被发现的服务，必须规避两层省电：
        //   ① 射频关停——持 network awake lock，空闲关射频策略
        //      wifi_idle_stop_allowed 检查 network_lock_active 自动 defer；
        //   ② modem sleep——set_discovery_services_power_policy 启用后，
