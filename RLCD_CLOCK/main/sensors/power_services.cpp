...
namespace {
// 常驻发现服务（MiPlay/airkan）电源策略。atomic：策略在事件处理器里写、
// 在多个网络任务里读，不能用普通 bool。
std::atomic<bool> s_discovery_always_on{false};
} // namespace

void set_discovery_services_power_policy(bool enabled)
{
    s_discovery_always_on.store(enabled, std::memory_order_release);
    ESP_LOGI(TAG, "discovery services power policy: %s",
             enabled ? "always-on (no modem sleep)" : "default");
}

bool wifi_always_on_required()
{
    return s_discovery_always_on.load(std::memory_order_acquire);
}
